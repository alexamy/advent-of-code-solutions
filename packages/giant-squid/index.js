...
const solve2 = (data) => {
  ...
  const { result: [number, winBoard] } = numbers.reduce((iter, number) => {
    if(iter.result) return iter;
    ...
    if(boards.every(isWinBoard)) {
      return { result: [number, boardWin] };
    };

    return { ...iter, boards: iter.boards.filter(b => !isWinBoard(b)) };
  }, { boards, result: null });
